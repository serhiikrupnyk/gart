...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres \
  ...
  -v app_db="$GART_DB" \
  -v shadow_db="$GART_DB_SHADOW" \
  -v test_db="$GART_DB_TEST" <<'EOSQL'
...
# keeps every database identical and means the app role never needs the privilege.
for database in "$GART_DB" "$GART_DB_SHADOW" "$GART_DB_TEST"; do
